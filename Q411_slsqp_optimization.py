"""
==========================================================================
Huashu Cup 2026 Problem B - Question 4 Q411
SLSQP (Sequential Least Squares Programming) Optimization
Optimal Investment Allocation for China's AI Competitiveness
==========================================================================

Method: SLSQP - Gradient-based constrained nonlinear optimization
Characteristics:
- Local optimization (may find local optimum)
- Fast convergence
- Handles equality and inequality constraints well
...
print("=" * 70)
print("Q411: SLSQP Optimization Method")
print("Sequential Least Squares Programming")
print("=" * 70)
...
def objective_slsqp(x):
    """
    SLSQP Objective: Maximize score with diversification bonus
    """
    score = marginal_benefit(x)
    
    # Diversification bonus (penalize concentration)
    shares = x / (np.sum(x) + 1e-10)
    hhi = np.sum(shares ** 2)  # Herfindahl index
    ...
    obj = score - 0.1 * hhi * 100 + 0.05 * weight_alignment * 100
    
    return -obj

# ==========================================================================
# 7. SLSQP Optimization
# ==========================================================================
print("\n--- 7. SLSQP Optimization ---")
...
constraints = [
    ...
    {'type': 'ineq', 'fun': lambda x: np.min(x) - min_bound},   # Min constraint
    {'type': 'ineq', 'fun': lambda x: max_bound - np.max(x)}    # Max constraint
]

# Initial guess based on dimension weights
x0 = np.array([dim_weights[name] for name in dim_names])
x0 = x0 / np.sum(x0) * TOTAL_BUDGET
...
print(f"Method: SLSQP (Sequential Least Squares Programming)")
...
print("\n" + "=" * 70)
print("Q411 SLSQP OPTIMAL INVESTMENT ALLOCATION")
print("=" * 70)
...
ax1.set_title('SLSQP Optimal Investment Allocation', fontsize=12, fontweight='bold')
...
ax2.set_title('Dimension Score Improvement', fontsize=12, fontweight='bold')
ax2.legend()
ax2.grid(True, alpha=0.3, axis='y')

# 3. Investment vs Weight comparison
ax3 = axes[1, 0]
inv_pct = optimal_x / TOTAL_BUDGET
weight_pct = np.array([dim_weights[name] for name in dim_names])

ax3.bar(x - width/2, inv_pct * 100, width, label='Investment %', color='steelblue')
ax3.bar(x + width/2, weight_pct * 100, width, label='Weight %', color='coral')

ax3.set_xticks(x)
ax3.set_xticklabels([dimensions[name]['name_cn'] for name in dim_names], rotation=30, ha='right')
ax3.set_ylabel('Percentage (%)')
ax3.set_title('Investment vs Dimension Weight', fontsize=12, fontweight='bold')
ax3.legend()
...
plt.suptitle('Q411: SLSQP Investment Optimization Results', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('Q411_slsqp_optimization.png', dpi=150, bbox_inches='tight')
print("Saved: Q411_slsqp_optimization.png")
plt.close()
...
results_df.to_csv('Q411_slsqp_results.csv', index=False)
print("Saved: Q411_slsqp_results.csv")

# Summary
summary_df = pd.DataFrame({
    'Metric': ['Method', 'Total_Budget', 'China_Score_2025', 'China_Score_2035', 
               'Score_Improvement', 'HHI_Index', 'Optimization_Success', 'Iterations'],
    'Value': ['SLSQP', TOTAL_BUDGET, china_baseline, china_final,
              china_final - china_baseline, hhi, result.success, result.nit]
})
summary_df.to_csv('Q411_slsqp_summary.csv', index=False)
print("Saved: Q411_slsqp_summary.csv")

# ==========================================================================
# Summary
# ==========================================================================
print("\n" + "=" * 70)
print("Q411 SLSQP OPTIMIZATION COMPLETE")
print("=" * 70)
print(f"""
Method: SLSQP (Sequential Least Squares Programming)
Characteristics:
  - Gradient-based local optimization
  - Fast convergence ({result.nit} iterations)
...
Output Files:
  - Q411_slsqp_results.csv
  - Q411_slsqp_summary.csv
  - Q411_slsqp_optimization.png
""")
print("=" * 70)
